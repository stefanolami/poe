...
type TenderEmailCharInProps = {
	...
	in_brief?: string
	deadline: DeadlineTuple[]
	further_details?: FurtherDetailsTuple[] | null
	tailored?: boolean
...
const TendersEmailCharin = ({
	...
	/* const hasAdditionalInfoColumn = deadline.length > 3 */
							...
							>
								<tbody>
									{deadline.map((d, i) => {
										...
										return (
											<tr key={i}>
												<td style={tdCell}>{note}</td>
												<td style={tdCell}>
													{dateStr}
												</td>
												<td style={tdCell}>{time}</td>
												{/* {d.length > 3 && (
												<td style={tdCell}>
													{note || ''}
												</td>
