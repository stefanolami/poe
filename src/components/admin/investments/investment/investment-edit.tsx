...
import { createInvestmentsSchema } from '@/lib/zod-schemas'
...
import InvestmentsDeadline from '../form-fields/investments-deadline'
import InvestmentsFurtherDetails from '../form-fields/investments-further-details'
import InvestmentsConsultant from '../form-fields/investments-consultant'
...
export const InvestmentEdit = ({
	...
	const form = useForm<CreateInvestmentsType>({
		resolver: zodResolver(createInvestmentsSchema),
		defaultValues: {
			geography: geography || [],
	...
	const submitHandler: SubmitHandler<CreateInvestmentsType> = async (
		data
	) => {
		console.log('Form data:', data)
		const formattedData: UpdateInvestmentType = {
			...data,
			oldFiles: filesArray,
		}
		try {
			const response = await updateInvestment(id, formattedData)
	...
	return (
		<div className="form w-full mb-16">
			<div className="grid grid-cols-2 items-start gap-4 text-white font-jose text-2xl mb-24">
				<h1 className="">Edit Investment</h1>
			</div>
			<Form {...form}>
				<form
